...
/*
    TODO: Implement syscall for heap operations
    bruh
*/

#define HEAP_SYSCALL_PLACEHOLDER_FUNCTION true
...
static int heap_init(void) {
    ...
    size_t initial_size = 0x100000;  // 1MB
    if (syscall_heap_expand((void*)HEAP_START_ADDR, initial_size) != 0) {
        return -1; 
    }
...
void* malloc(size_t size) {
    ...
    if (!block) {
        if (heap_expand(size + sizeof(block_header_t)) != 0) {
            return NULL;  // OOM
        }
        block = find_free_block(size);
        if (!block) {
            return NULL;  // block occupied
        }
    }
